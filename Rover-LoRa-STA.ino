  ...
  int stepsPerRev = 3840;    // 1 revolución por defecto, cambiar con .STEP###
  #define STEP_DELAY_US 150   // Velocidad

  bool shtPresent = false;
  bool ahtPresent = false;
  bool inaESP_Present = false;
  bool inaM1_Present = false;
  bool inaM2_Present = false;
  bool mpuPresent = false;
  bool dist1Present = false;
  bool dist2Present = false;
  bool dist3Present = false;
  ...
  float intervalSec = 1.5;                      // interval in seconds (default)
  ...
  void motorTask(void * parameter) {
    const int turnSteps = 3840;  // Giros de 90 grados
    ...
    while (true) {
      if (currentCommand != MOTOR_IDLE) {
        ...
        int stepsToRun = stepsPerRev;  // default

        switch (currentCommand) {
          case MOTOR_BOTH_CW:
            dir1 = LOW;
            dir2 = LOW;
            stepsToRun = stepsPerRev;
            break;
